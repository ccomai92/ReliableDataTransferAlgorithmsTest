...
int main(int argc, char *argv[])
{
	...
	if (myPart == CLIENT)
	{
		...
		case 2:
			...
			break;
		case 3: // modified for case #4  
			// In client side, 
			// For sliding window with size 1, running error rate starting from 0 to 10%
			// measure elapsed time, and retransmits for every error rate. 
			// print droprate and time elapsed to cout, otherwise print to cerr
            cerr << "WindowSize = " << 1 << endl; 
            for (int dropRate = 0; dropRate <= 10; dropRate++) {    
                timer.start(); // start timer
				retransmits =
					clientSlidingWindow(sock, MAX, message, 1); // actual test
				cerr << "Drop rate (%) = ";								 // lap timer
				cout << dropRate << " ";
				cerr << "Elasped time = ";
				cout << timer.lap() << endl;
				cerr << "retransmits = " << retransmits << endl;
			...
			// For sliding window with size 30, running error rate starting from 0 to 10%
			// measure elapsed time, and retransmits for every error rate. 
			// print droprate and time elapsed to cout, otherwise print to cerr
            cerr << "WindowSize = " << 30 << endl; 
            for (int dropRate = 0; dropRate <= 10; dropRate++) {    
                timer.start(); // start timer
				retransmits =
	...
	if (myPart == SERVER)
	{
		...
		case 3: // modified for case #4 
			// For server side, 
			// for sliding window size 1, running server for every error rate 
			// starting from 0 to 10% 
            for (int dropRate = 0; dropRate <= 10; dropRate++) {
			    serverEarlyRetrans(sock, MAX, message, 1, dropRate); 
            } 
        
			// for sliding window size 30, running server for every error rate 
			// starting from 0 to 10% 
            for (int dropRate = 0; dropRate <= 10; dropRate++) {
			    serverEarlyRetrans(sock, MAX, message, 30, dropRate); 
            } 
			break;
		default:
			cerr << "no such test case" << endl;

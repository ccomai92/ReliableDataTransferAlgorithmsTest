...
// Client has window size that is synchronized with the server. Client sends message
// as long as his sequence number is samller than the max window size. If the client 
// receives acknowledgement from the server that matches the sendBase index, it slides
// the window. 
...
// Server receives messages from the client and save the message received. 
// Server's expecting sequence numbers are in the sliding window scope. 
// Everytime, server receives the message,it saves the message and send ack  
...
void serverEarlyRetrans(UdpSocket &sock, const int max, int message[], int windowSize)
{
	...
	// once receives the last message 19999 from the client, 
	// server ends its session 
	while (rcvBase < max - 1)
	{
		// receive message from the client. 
		sock.recvFrom((char *)message, MSGSIZE);
		int sequence = message[0];
		// cerr << temp << endl;

		// if the sequence of message is within the window
		// boundary, it saves the message 
		if (sequence > rcvBase && sequence < endIndex)
		{
			savedMessage[sequence] = true;
		}

		// if the sequence of message is what server is 
		// expecting to slide the window, it slides the 
		// window 
		if (sequence == (rcvBase + 1))
		{
		...
		// seding ack message to client, which contains the 
		// last sequence number received in order. 
		message[0] = rcvBase;
